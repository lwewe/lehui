...
<script>
import { cxpay_success, czwxpay, getopenid, getPackage, CardChongzhi, getLogo } from "@/api/lhcard";
...
export default {
  ...
  methods: {
    ...
    //         "timeStamp":params.timestamp.toString(),//支付签名时间戳，注意微信jssdk中的所有使用timestamp字段均为小写。但最新版的支付后台生成签名使用的timeStamp字段名需大写其中的S字符
    ...
    onBridgeReady(params, order_no) {
      ...
      WeixinJSBridge.invoke(
        ...
        function (res) {
          // 无论用户是支付成功、取消支付还是支付失败，微信都会回调这个函数
          // 通过 res.err_msg 判断实际状态
          if (res.err_msg === "get_brand_wcpay_request:ok") {
            ...
            cxpay_success({
              ...
              that.$toast(res.msg || "充值成功")
              if (res.code == 200) {
                // ✅ 核心：成功之后返回上一页（go(-1) 正好返回福利卡详情页）
                setTimeout(() => {
                  that.$router.go(-1)
                }, 800)
              }
