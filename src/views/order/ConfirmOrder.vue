<template>
  <div class="location">
    ...
    <!-- 地址 -->
    <div class="addressBox" @click="changeAdress">
      <div class="address">
      ...
      <div class="platform-summary">
        <div class="orderInfo">
          <div class="shopPrice">
            ...
            <!-- ￥{{ Number(group.goodsTotal).toFixed(2) }} -->
            <div class="price"><span style="font-size: 10px;">￥</span>{{ Number(group.goodsTotal).toFixed(2) }}</div>
          </div>
          <div class="shopPrice">
...
import { orderDetail, cardList, cartList, orderBuyNow, orderSubmit, payByCard, addressList, userInfo, orderSettle } from "@/api/lhjdtm";
...
export default {
  ...
  methods: {
    ...
    changeAdress() {
      console.log('跳地址前 dataDetail:', JSON.stringify(this.dataDetail))
    ...
    // 获取地址列表，默认选中默认地址
    getAddress() {
      return addressList().then(res => {
        if (res.code == 200 && res.data && res.data.length > 0) {
          let target = null

          if (this.orderDeliveryId) {
            target = res.data.find(item => item.id === this.orderDeliveryId)
          }
          if (!target) {
            target = res.data.find(item => item.is_default == 1) || res.data[0]
          }
        ...
        if (this.deliveryInfo) {
          const d = this.deliveryInfo
          this.addrShow = { id: d.id, name: d.name, phone: d.phone, addr: d.addr || '' }
        } else {
          this.addrShow = null
        }
      })
    ...
    getCheckout() {
      const params = {
        address_id: this.addrShow ? this.addrShow.id : 0
      }
      // ✅ 多平台聚合单必须优先 group_id，否则只恢复一个子订单
      if (this.group_id) {
        params.group_id = Number(this.group_id)
      } else if (this.order_id) {
        params.order_id = Number(this.order_id)
      } else if (this.dataDetail.cartIds) {
        params.cartIds = Array.isArray(this.dataDetail.cartIds)
          ? this.dataDetail.cartIds.join(',')
          : this.dataDetail.cartIds
      }

      if (this.group_id || this.order_id || this.dataDetail.cartIds) {
        orderSettle(params).then(res => {
          if (res.code == 200) {
            if (res.data.order_id) this.order_id = res.data.order_id
            if (res.data.group_id) this.group_id = res.data.group_id

            this.groupedOrderList = res.data.groups.map((group, gIdx) => ({
              platform: group.platform,
              platform_name: group.platform_name,
              items: (group.items || []).map((it, idx) => ({
                id: it.id || `${group.order_no || gIdx}_${it.product_id}_${idx}`,
              ...
              totalPrice: group.sub_total,
              goodsTotal: group.goods_total,
              freight: group.freight ?? '0.00',
              count: group.item_count
            }))

            this.orderList = this.groupedOrderList.flatMap(g => g.items)
            this.total = res.data.pay_amount
            this.loadingflag = false
          } else {
            this.loadingflag = false
            this.$toast(res.msg || '获取结算信息失败')
          }
        }).catch(() => {
          this.loadingflag = false
          this.$toast('网络异常')
        ...
        // 立即购买（无 cartIds）走本地拼装
        let detail = this.dataDetail
        this.orderList = [{
          id: 0,
          product: {
            title: detail.product_title,
            img: detail.product_img,
            sell_price: detail.sell_price,
            platform: detail.platform || 2
          },
          title: detail.product_title,
          img: detail.product_img,
          sell_price: detail.sell_price,
        ...
        this.total = Number(detail.sell_price) * Number(detail.quantity)
        this.groupOrderList()
        this.loadingflag = false
      }
    },
    ...
    toPay() {
      ...
      if (!this.addrShow || !this.addrShow.id) {
        this.$toast("请先选择收货地址")
        return
      }
  ...
  created() {
    if (this.$route.query.order_id || this.$route.query.group_id) {
      this.order_id = this.$route.query.order_id
      this.group_id = this.$route.query.group_id || ''   // ✅ 保留 group_id
      this.total = Number(this.$route.query.total_price || 0)
      this.getAddress().then(() => {
        this.getCheckout()   // ✅ 走 orderSettle，传 order_id
      })
      this.getCard()
      this.checkPayPwd()
      return
    ...
    let cacheData = sessionStorage.getItem('confirmOrderData')
    this.dataDetail = this.$route.query.data
    ...
    if (this.$route.query.cartIds) {
      this.dataDetail.cartIds = this.$route.query.cartIds
    }

    const selectedId = sessionStorage.getItem('selectedAddressId')
    if (selectedId) {
      this.orderDeliveryId = Number(selectedId)
