    ...
    <div class="order-content">
      ...
      <div v-else-if="orderList.length > 0" class="order-list">
        <div class="order-card" v-for="(item, index) in orderList" :key="item.id" @click="goDetail(item.id)">
          ...
          <div class="card-header">
            <div class="order-no">
              <span class="platform-tag">
                ...
                {{ item.platform_name || getPlatformName(item.platform, item.jd_type) }}
          ...
          <!-- 主体：商品信息 -->
          <div class="card-body">
            <div class="product-item" v-for="(goods, gIndex) in item.items" :key="gIndex">
...
export default {
  ...
  methods: {
    ...
    // 统一订单数据结构
    normalizeOrder(item) {
      // 京东/天猫多商品：已经有 items 数组
      if (Array.isArray(item.items) && item.items.length) {
        return {
          ...item,
          total_price: item.total_price ?? item.pay_amount ?? item.total_fee ?? 0,
          items: item.items.map(g => ({
            product_img: g.product_img,
      ...
      // 鲜花：商品信息在顶层
      if (item.biz === 'jdtm' && item.platform === 5) {
        return {
          ...item,
          total_price: item.total_price ?? item.pay_amount ?? item.total_fee ?? 0,
          items: [{
    ...
    handleAction(action, item, event) {
      ...
      if (action === 'payOrder') {
        // ✅ 加日志确认 item.id 是当前点击卡的 id
        console.log('点击支付的 item.id:', item.id, 'item:', JSON.stringify(item))

        const firstItem = (item.items && item.items[0]) || {};
        const payAmount = item.total_price ?? item.pay_amount ?? item.total_fee ?? firstItem.sell_price ?? 0;

        sessionStorage.setItem('orderProductInfo', JSON.stringify({
          product_title: item.product_title || item.product?.title || firstItem.product_title,
          product_img: item.product_img || item.product?.img || firstItem.product_img,
          sell_price: payAmount,
          quantity: item.quantity ?? firstItem.quantity ?? 1,
    ...
    async fetchOrderList() {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          // 如果列表为空，显示空状态
          const normalized = list.map(item => this.normalizeOrder(item));
          if (this.page === 1) {
            this.orderList = normalized;
          } else {
            this.orderList = [...this.orderList, ...normalized];
          }
          this.total = total;
