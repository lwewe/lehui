<template>
  <div class="pageMax">
    ...
    <div v-for="(item, index) in prouctList" :key="item.id || item.order?.id || index">
      ...
      <div>
        ...
        <!-- add景区 -->
        <!--          电影-->
        <div class="listItem" v-else-if="item.ord_bs == 1" @click="tomoveOrder(item.id)">
          <div class="orderNo">
        ...
        <!--    卡券-->
        <div class="listItem" v-else-if="item.ord_bs == 2" @click="tocardOrder(item.id)">
          <div class="orderNo">
        ...
        <!--    商品-->
        <div class="listItem" @click="toShopOrder(item.id)" v-else-if="item.ord_bs == 3">
          <div class="orderNo">
            <div class="orderNum orderTime">订单号:{{ item.order_no }}</div>
            <!--    订单状态-->
            <div v-if="item.state">
        ...
        <!--      肯德基-->
        <!--    点餐-->
        <div class="listItem" v-else-if="item.ord_bs == 4"
          @click="toorderOrder(item.id, item.pf, item.storeId, item.type)">
          <div class="orderNo">
        ...
        <!--        蛋糕-->
        <!--        蛋糕/电子券-->
        <template v-else-if="item.ord_bs == 5 || (item.order && item.order.ord_bs == 5)">
          <!--   新增fff：电子券 flag == 2（独立判断，不需要 order_list） -->
          <div class="listItem" v-if="item.order && item.order.flag == 2" @click="toCouponDetail(item.order.id)">
            <div class="orderNo">
            ...
            <div class="priceBox" style="margin-top: 3px">
              ...
              <div>
                总计: <span class="priceNum"><span style="font-size: 12px">￥</span>{{
                  Number(item.order.zongji).toFixed(2)
          ...
          <div v-if="item.order_list">
            <div class="listItem" v-if="item.order && item.order.flag == 3">
              <div v-for="item2 in item.order_list" :key="item2.id" @click="toDetail(item2.id)">
                <div class="orderNo">
                  ...
                  <div v-if="item2.flag == 3">
                    <div class="otherType" v-if="item2.detail && item2.detail.status == 3">已取消</div>
                    <div class="otherType" v-else-if="item2.detail && item2.detail.status == 2">已完成</div>
                    <div class="otherType" v-else-if="item2.detail && item2.detail.status == 1">已确认</div>
                    <div class="otherType" v-else-if="item2.detail && item2.detail.status == 0">待确认</div>
                  </div>
                  <div v-else-if="item2.flag == 4">
                    <div class="otherType" v-if="item2.state == 4">已取消</div>
                    <div class="otherType" v-else-if="item2.state == 3">已完成</div>
                    <div class="otherType" v-else-if="item2.state == 2">已发货</div>
                    <div class="otherType" v-else-if="item2.state == 1">已确认</div>
                    <div class="otherType" v-else-if="item2.state == 0">待确认</div>
                  </div>
                </div>
                <div class="filmBox"
                  v-for="(item3, index3) in (item2.flag == 4 ? (item2.content || []) : (item2.detail ? item2.detail.product : []))"
                  ...
                  <div class="film">
                    ...
                    <div class="rightBox">
                      ...
                      <div class="startTime textBox" style="margin-top: 0px">{{
                        item2.flag == 4 ? (item3.xinghao ? item3.xinghao.name : '') : item3.spec_name }}</div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="priceBox" style="margin-top: 3px">
                ...
                <div>
                  总计: <span class="priceNum"><span style="font-size: 12px">￥</span>{{
                    item.order.zongji ? item.order.zongji : (item.allPrice || 0).toFixed(2)
                  }}</span><span class="first">元</span>
                </div>
              </div>
            </div>
          ...
          <!-- 直接蛋糕对象（无 order 包裹，flag == 3 或 flag == 4） -->
          <div class="listItem" v-else-if="item.flag == 3 || item.flag == 4" @click="toDetail(item.id)">
            <div class="orderNo">
              <div class="orderNum orderTime">订单号:{{ item.order_no }}</div>
              <div v-if="item.flag == 3">
                <div class="otherType" v-if="item.detail && item.detail.status == 3">已取消</div>
                <div class="otherType" v-else-if="item.detail && item.detail.status == 2">已完成</div>
                <div class="otherType" v-else-if="item.detail && item.detail.status == 1">已确认</div>
                <div class="otherType" v-else-if="item.detail && item.detail.status == 0">待确认</div>
              </div>
              <div v-else-if="item.flag == 4">
                <div class="otherType" v-if="item.state == 4">已取消</div>
                <div class="otherType" v-else-if="item.state == 3">已完成</div>
                <div class="otherType" v-else-if="item.state == 2">已发货</div>
                <div class="otherType" v-else-if="item.state == 1">已确认</div>
                <div class="otherType" v-else-if="item.state == 0">待确认</div>
              </div>
            </div>
            <div class="filmBox"
              v-for="(item3, index3) in (item.flag == 4 ? (item.content || []) : (item.detail ? item.detail.product : []))"
              :key="index3" style="margin-bottom: 5px">
              <div class="film">
                <div class="filmImg">
                ...
                <div class="rightBox">
                  ...
                  <div class="startTime textBox" style="margin-top: 0px">{{
                    item.flag == 4 ? (item3.xinghao ? item3.xinghao.name : '') : item3.spec_name }}</div>
                </div>
              </div>
            </div>
            <div class="priceBox" style="margin-top: 3px">
              <div>共{{ (item.detail && item.detail.product) ? item.detail.product.length : (item.content ?
                item.content.length : 0) }}件商品</div>
              <div>
                总计: <span class="priceNum"><span style="font-size: 12px">￥</span>{{
                  item.flag == 4 ? (item.order ? item.order.zongji : '0.00') : (item.detail ? item.detail.final_amount :
                    '0.00')
                }}</span><span class="first">元</span>
              </div>
            </div>
        ...
        <!--        图书-->
        <!--        图书-->
        <div class="listItem" v-else-if="item.ord_bs == 6" @click="toBookOrder(item.id)">
          <div class="orderNo">
            <div class="orderNum orderTime">订单号:{{ item.ordersn }}</div>
            <div>
              <div class="otherType" v-if="item.state == 0">待发货</div>
              <div class="otherType" v-else-if="item.state == 1">待发货</div>
              <div class="otherType" v-else-if="item.state == 2">已发货</div>
              <div class="otherType" v-else-if="item.state == 3">处理中</div>
              <div class="otherType" v-else-if="item.state == 4">已取消</div>
              <div class="otherType" v-else-if="item.state == 5">已完成</div>
            </div>
          </div>
          <div class="filmBox" v-for="(item2, i) in (item.info_list || [])" :key="i">
            <div class="film">
              <div class="filmImg cardImg">
        ...
        <div class="listItem" v-else-if="item.ord_bs == 7" @click="toPerOrder(item.id)">
          <div class="orderNo">
            <div class="orderNum">订单号: <span>{{ item.order_id }}</span></div>
            <!--    订单状态-->
            <div>
        ...
        <!--        直充-->
        <div class="listItem" v-else-if="item.ord_bs == 8" @click="toChargeDetail(item.id)">
          <div class="orderNo">
            <div class="orderNum orderTime">订单号:{{ item.order_no }}</div>
            <!--    订单状态-->
