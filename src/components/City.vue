<template>
  <div>
    <div class="address" @click="showPopup()">
      <div class="city">{{ formatCity(city) }}</div><img src="../assets/pull1.png" style="width: 12px;" alt="">
      <!-- <div class="pull-down">
...
export default {
  ...
  methods: {
    ...
    if (typeof city === 'object' && city !== null) {
      return city.name || '正在定位';
    }
    if (city === '[object Object]') {
      return '正在定位';
    }
    return city || '正在定位';
  },
...
created() {
  ...
  // 检查存储的值是否为有效字符串
  if (storedCity && 
      storedCity !== "undefined" && 
      storedCity !== "null" && 
      storedCity !== "[object Object]") {
    
    // 尝试解析，防止存储的是 JSON 字符串
    try {
      let parsedCity = JSON.parse(storedCity);
      if (typeof parsedCity === 'object' && parsedCity.name) {
        this.city = parsedCity.name;
      } else {
...
updated() {
  let storedCity = sessionStorage.getItem("cityName");
  if (storedCity && storedCity !== "[object Object]") {
    this.city = storedCity;
  }
}
}
